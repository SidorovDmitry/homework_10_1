...
def test_correct_format(correct_date_string):
    assert get_date(correct_date_string) == "11.03.2024"


def test_incorrect_format_no_time(incorrect_date_string_no_time):
    assert get_date(incorrect_date_string_no_time) == "Некорректный формат даты"


def test_incorrect_format_wrong_separator(incorrect_date_string_wrong_separator):
    assert get_date(incorrect_date_string_wrong_separator) == "Некорректный формат даты"


def test_incorrect_format_extra_chars(incorrect_date_string_extra_chars):
    assert get_date(incorrect_date_string_extra_chars) == "Некорректный формат даты"


def test_incorrect_format_non_numeric(incorrect_date_string_non_numeric):
    assert get_date(incorrect_date_string_non_numeric) == "Некорректный формат даты"


def test_incorrect_format_wrong_delimiter(incorrect_date_string_wrong_delimiter):
    assert get_date(incorrect_date_string_wrong_delimiter) == "Некорректный формат даты"


def test_incorrect_format_too_many_parts(incorrect_date_string_too_many_parts):
    assert get_date(incorrect_date_string_too_many_parts) == "Некорректный формат даты"


def test_incorrect_format_invalid_month(incorrect_date_string_invalid_month):
